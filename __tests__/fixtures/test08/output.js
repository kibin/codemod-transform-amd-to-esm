...
import generateDefaultName from "utils/generateDefaultName";
import backendValidation from "base/backendValidation";
import notify from "ui/notify";
import i18n from "i18n";
import __fetchLimit from "appConfig";
let SamplesModel;
const samplesL = i18n.getBundle().redactor.samples;
const fetchLimit = __fetchLimit.PAGE_SIZE;

class Sample extends Backbone.Model {
  static initClass() {
    this.prototype.defaults = { isSelected: false };
  }
  ...
  markShapes() {
    return __guard__(
      __guard__(this.get("content"), (x1) => x1.shapes),
      (x) => x.forEach((shape) => (shape.previewInSamplePanel = true))
    );
  }
}
Sample.initClass();
...
class CustomSamples extends Backbone.Collection {
  ...
  fetch() {
    return super
      .fetch({ url: this.url + "?limit=" + fetchLimit })
      .then(({ data }) => {
        this.isFetched = true;
        this.markShapes();
...
class DefaultSamples extends Backbone.Collection {
  ...
  constructor(samples, { samplesModel }) {
    ...
    this.isFetched = true;
    this.reset(
      Samples.map(function (sampleData) {
        const data = {
...
export default SamplesModel = class SamplesModel extends Backbone.Model {
  initialize({ redactor }) {
  ...
  saveAsSample() {
    return this.redactor
      ...
      .fail(
        backendValidation(({ error }) => notify.error(i18n.getMessage(error)))
      )
      .then((response) => {
  ...
  start() {
    return this.redactor
      .getPage()
      .startChanges({ onFinishChanges: () => this.onFinishChanges() });
  }
  ...
  loadNextCustomPage() {
    return $.ajax({
      type: "GET",
      url:
        this.customSamples.url +
        "?limit=" +
        fetchLimit +
        "&offset=" +
        this.customSamples.length,
      contentType: "application/json",
    })
      .fail(
        backendValidation(({ error }) => notify.error(i18n.getMessage(error)))
